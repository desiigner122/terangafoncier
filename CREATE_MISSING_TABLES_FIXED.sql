...
-- VERSION CORRIGÉE - Vérifie auth.users avant création
-- ============================================

-- Vérifier d'abord que auth.users existe
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'La table auth.users n''existe pas. Vérifiez votre configuration Supabase Auth.';
  END IF;
END $$;
...
ALTER TABLE blockchain_certificates 
  ADD CONSTRAINT fk_blockchain_certificates_user 
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- Ajouter la contrainte property_id seulement si la table properties existe
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'properties') THEN
    ALTER TABLE blockchain_certificates 
      ADD CONSTRAINT fk_blockchain_certificates_property 
...
DROP POLICY IF EXISTS "Users can view own certificates" ON blockchain_certificates;
CREATE POLICY "Users can view own certificates"
  ON blockchain_certificates FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own certificates" ON blockchain_certificates;
CREATE POLICY "Users can insert own certificates"
  ON blockchain_certificates FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own certificates" ON blockchain_certificates;
...
ALTER TABLE blockchain_transactions 
  ADD CONSTRAINT fk_blockchain_transactions_certificate 
  FOREIGN KEY (certificate_id) REFERENCES blockchain_certificates(id) ON DELETE SET NULL;

DO $$ 
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'properties') THEN
    ALTER TABLE blockchain_transactions 
      ADD CONSTRAINT fk_blockchain_transactions_property 
      FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE SET NULL;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can insert own transactions" ON blockchain_transactions;
...
-- ============================================
DROP TABLE IF EXISTS calendar_appointments CASCADE;
...
ALTER TABLE calendar_appointments 
  ADD CONSTRAINT fk_calendar_appointments_user 
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE calendar_appointments 
  ADD CONSTRAINT fk_calendar_appointments_created_by 
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'properties') THEN
    ALTER TABLE calendar_appointments 
      ADD CONSTRAINT fk_calendar_appointments_property 
      FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE SET NULL;
  END IF;
  
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'purchase_requests') THEN
    ALTER TABLE calendar_appointments 
...
DROP POLICY IF EXISTS "Users can view own appointments" ON calendar_appointments;
CREATE POLICY "Users can view own appointments"
  ON calendar_appointments FOR SELECT
  USING (auth.uid() = user_id OR auth.uid() = created_by);

DROP POLICY IF EXISTS "Users can insert own appointments" ON calendar_appointments;
CREATE POLICY "Users can insert own appointments"
  ON calendar_appointments FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own appointments" ON calendar_appointments;
CREATE POLICY "Users can update own appointments"
  ON calendar_appointments FOR UPDATE
  USING (auth.uid() = user_id OR auth.uid() = created_by);

DROP POLICY IF EXISTS "Users can delete own appointments" ON calendar_appointments;
...
-- ============================================
DROP TABLE IF EXISTS documents_administratifs CASCADE;
...
ALTER TABLE documents_administratifs 
  ADD CONSTRAINT fk_documents_administratifs_user 
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

ALTER TABLE documents_administratifs 
  ADD CONSTRAINT fk_documents_administratifs_uploaded_by 
  FOREIGN KEY (uploaded_by) REFERENCES auth.users(id);

ALTER TABLE documents_administratifs 
  ADD CONSTRAINT fk_documents_administratifs_reviewed_by 
...
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'properties') THEN
    ALTER TABLE documents_administratifs 
      ADD CONSTRAINT fk_documents_administratifs_property 
      FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE SET NULL;
  END IF;
  
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'purchase_requests') THEN
    ALTER TABLE documents_administratifs 
      ADD CONSTRAINT fk_documents_administratifs_purchase_request 
      FOREIGN KEY (purchase_request_id) REFERENCES purchase_requests(id) ON DELETE SET NULL;
  END IF;
END $$;
...
CREATE INDEX idx_documents_administratifs_property_id ON documents_administratifs(property_id);
CREATE INDEX idx_documents_administratifs_status ON documents_administratifs(status);
