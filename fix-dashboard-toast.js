...
// Fichier: fix-dashboard-toast.js
...
const filesToFix = [
  ...
  'src/pages/dashboards/particulier/MyLandsPage.jsx',
  'src/pages/dashboards/particulier/RequestsPage.jsx',
  'src/pages/dashboards/particulier/DigitalVaultPage.jsx',
  'src/pages/dashboards/particulier/ProfilePage.jsx'
];
  ...
  try {
    if (fs.existsSync(fullPath)) {
      ...
      if (content.includes('useToast') || content.includes('toast(')) {
        ...
        // Ajouter le système safeToast au début du composant
        ...
        content = content.replace(/toast\s*\(\s*{([^}]+)}\s*\)/g, (match, params) => {
          ...
          return `safeToast(${typeof description === 'string' && !description.startsWith('${') ? `"${description}"` : description}, ${typeof variant === 'string' && !variant.startsWith('${') ? `"${variant}"` : variant})`;
        });
