    ...
    <div className="max-w-5xl mx-auto space-y-6">
      { (context.parcelleId || context.projectId) && (
        <div className="p-3 rounded-md bg-blue-50 border border-blue-200 text-sm text-blue-800 flex items-center justify-between">
          <div>
            Contexte importé: {context.parcelleId ? `Parcelle #${context.parcelleId}` : `Projet ${context.projectId}`} · Méthode: paiement comptant
          </div>
          {backLink && (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Banknote className="h-5 w-5 text-primary"/> Paiement Comptant</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-sm text-muted-foreground">
          ...
          <p>Finalisez l'achat d'un terrain en un seul versement. Sélectionnez un bien, recevez les documents, et payez comptant.</p>
          <ul className="list-disc pl-5">
            <li>Sélection du terrain (marché des vendeurs ou promoteurs)</li>
            <li>Vérification documentaire (notaire/mairie selon cas)</li>
            <li>Paiement sécurisé (escrow / virement)</li>
          </ul>
        </CardContent>
      </Card>
    </div>
