      ...
      <Card className="h-full flex flex-col overflow-hidden bg-card shadow-md hover:shadow-xl transition-all duration-300 border border-border/50 rounded-xl group">
        <CardHeader className="p-0 relative">
          <Link to={`/parcelles/${parcel.id}`} className="block aspect-[16/10] bg-muted overflow-hidden relative">
            <img  alt={parcel.name || 'Image de parcelle'} className="w-full h-full object-cover transition-transform duration-500 ease-in-out group-hover:scale-105" src={(parcel.images && parcel.images[0]) || "https://images.unsplash.com/photo-1572402230263-c9b91177908d"} />
        ...
        <CardContent className="p-4 flex-grow">
          <div className="flex justify-between items-start mb-2">
             <Badge variant={parcel.verified ? 'outline_success' : 'secondary'} className="text-xs flex items-center gap-1">
              <ShieldCheck className="h-3 w-3" /> {parcel.verified ? 'Vérifié' : 'En vérification'}
            </Badge>
          ...
          <div className="flex w-full gap-2">
            <Button asChild className="w-full">
              <Link to={`/parcelles/${parcel.id}`}>
                ...
                {isMunicipal ? 'Faire une demande' : 'Détails'}
              </Link>
            </Button>
