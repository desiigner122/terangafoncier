...
const BanqueTransactions = () => {
  ...
  useEffect(() => {
    ...
    const loadTransactions = async () => {
      try {
        // Transactions RÉELLES depuis Supabase (table financial_transactions)
        ...
        const mapped = (data || []).map((t) => ({
          id: t.id,
          type: t.type || t.transaction_type || 'credit',
        ...
        if (active) { setTransactions(mapped); setFilteredTransactions(mapped); }
      } catch (err) {
        console.warn('Transactions banque indisponibles:', err?.message);
        if (active) { setTransactions([]); setFilteredTransactions([]); }
      }
    };
    loadTransactions();
    return () => { active = false; };
